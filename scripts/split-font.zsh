...
@font-face {
    ...
    font-style: normal;
    font-weight: 600;
    src: local(GenYoMin TW), local(GenYoMinTW-SB), url('../cdn/$1') format('woff2');
    unicode-range: $2;
}
...
for ((i = 1; i <= $#RANGES; i++)); do
  ...
  pyftsubset ./GenYoMin-TW-SB.ttf --unicodes="${RANGES[i]}" --flavor="woff" --output-file="../public/fonts/$SUBSET_NAME"
